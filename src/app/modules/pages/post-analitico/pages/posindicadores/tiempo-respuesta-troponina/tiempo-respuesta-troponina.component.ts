...
@Component({
  selector: 'app-tiempo-respuesta-troponina',
  templateUrl: './tiempo-respuesta-troponina.component.html',
  styleUrls: ['./tiempo-respuesta-troponina.component.css']
})
export class TiempoRespuestaTroponinaComponent implements OnInit {
 ...
 getFiltro(){
  ...
  if(this.formulario.valid){
    ...
    this.IndicadoresService.getFiltroIndicadoresTres(this.formulario.value.mes,this.formulario.value.anio, this.formulario.value.turno, this.formulario.value.area).then((res:any)=>{
      console.log("Respuesta",res);
       this.valItem1 = res[0].responsetimetroponin
    ...
    // --------crear editar--------
      crearEditar(){
        ...
        if(this.indPatient){
          ...
          this.indPatient.responsetimetroponin = this.valItem1,
          console.log('Editar' , this.indPatient);
           this.IndicadoresService.updateIndicadoresTres(this.indPatient, this.indPatient.idtroponinresponsetime).subscribe(respuesta => {
            //this.spinner.hide();
            const Loguser = {
       ...
       if (!this.formulario.valid || this.formulario.get('mes').value == '--' || this.formulario.get('anio').value == '--') {
          ...
          let indJson = {
              ...
              responsetimetroponin: this.valItem1,
              ACTIVE : 1
          }
          ...
          this.IndicadoresService.createIndicadoresTres(indJson).subscribe(resp =>{
