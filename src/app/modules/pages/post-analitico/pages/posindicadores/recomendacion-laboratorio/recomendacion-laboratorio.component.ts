...
@Component({
  selector: 'app-recomendacion-laboratorio',
  templateUrl: './recomendacion-laboratorio.component.html',
  styleUrls: ['./recomendacion-laboratorio.component.css']
})
export class RecomendacionLaboratorioComponent implements OnInit {
  ...
  getFiltro(){
   console.log("Filtro")
    if(this.formulario.valid){
      ...
      this.IndicadoresService.getFiltroIndicadoresSiete(this.formulario.value.mes,this.formulario.value.anio).then((res:any)=>{
  ...
  filtroMesAno(){
   ...
   if(this.alert == false){
    ...
    console.log('Editar' , this.indPatient);
     this.IndicadoresService.updateIndicadoresSiete(this.indPatient, this.indPatient.idlaboratoryrecommend).subscribe(respuesta => {
      //this.spinner.hide();
      const Loguser = {
 ...
 if (!this.formulario.valid || this.formulario.get('mes').value == '--' || this.formulario.get('anio').value == '--') {
    ...
    console.log(indJson);
    this.IndicadoresService.createIndicadoresSiete(indJson).subscribe(resp =>{
      const Loguser = {
        Fecha: this.datePipe.transform(new Date(), "yyyy-MM-dd"),
        Hora: this.dateNowISO,
        Metodo: this.translate.instant('MODULES.NOTIFICACIONES.METODOLOGACTUALIZACION'),
    ...
    setTotalProgressBar(element){
      ...
      if(this.totalOrdPB_Tmp <= 100)
      {
        ...
        this.alert = true
        this.mensaje = this.translate.instant('El valor de Satisfacción de los servicios de laboratorio  debe ser menor a 100')
