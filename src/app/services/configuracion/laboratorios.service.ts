...
export class LaboratoriosService extends ApiService {
  ...
  getLogoImage(){
    ...
    return this.httpClient.get<any>(this.apiURL, { headers: reqHeaders })
    .pipe(
      map(
        (resp: any) => {

          if(!resp){
            return  this.noImgLogo;
          }
          if(resp.length > 0){
            return resp[0].logo;
          }
  ...
  getLogoImageClient(header : string){
    const reqHeaders = new HttpHeaders({ 'Content-Type': 'application/json', 'Client': header });
    return this.httpClient.get<any>(this.apiURL, { headers: reqHeaders })
    .pipe(
      map(
        (resp: any) => {
  ...
  public async getByIdlab(id: any) {
